    ...
    for track in tracks:
        ...
        if control_coord < up_line_out_position:
            if id in temp_down_list:
                ...
                mqttSender.sendMessage(f"{mqttClient}/{down_direction}/{up_direction}/face", str(datetime.datetime.now()), qos = 2)

        elif control_coord > down_line_out_position:
            if id in temp_up_list:
                temp_up_list.remove(id)
                mqttSender.sendMessage(f"{mqttClient}/{up_direction}/{down_direction}/face", str(datetime.datetime.now()), qos = 2)
        activeIDs.append(id)
...
def process_image(detections, conf_threshold, image: NpImage) -> Sequence[Detection]:
    out_detections = []
...
def realTime():
    ...
    while True:
        ...
        detections = process_image(detections, confThreshold, img)
